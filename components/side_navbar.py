...
def side_navbar_link(text, href, style = None):
    """A link to another dashboard"""
    return html.Li(
        dcc.Link(text, href=href, className="govuk-link govuk-link--no-visited-state", style = style),
        className="moj-side-navigation__item govuk-!-margin-bottom-1",
    )


def side_navbar_link_active(text, href, style = None):
    """
